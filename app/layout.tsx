...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { return <html lang="en" suppressHydrationWarning><head><script dangerouslySetInnerHTML={{__html:`try{const t=localStorage.getItem('vayrox-theme')||localStorage.getItem('vayxo-theme')||'system';document.documentElement.classList.toggle('dark',t==='dark'||(t==='system'&&matchMedia('(prefers-color-scheme:dark)').matches))}catch{}`}}/></head><body><AuthProvider><AuthGate><RouteShell>{children}</RouteShell><ConnectionStatus/><AiDock/><CookieBanner/></AuthGate></AuthProvider></body></html>; }
